...
public class ManagementExtension implements Extension {
  ...
  /**
   * This method gets called when an injection target is processed on bootstrap.
   * It wraps the InjectionTarget to add the JMX registration magic.
   * 
   * @param <T>
   * @param pit
   */
  public <T> void processInjectionTarget(@Observes ProcessInjectionTarget<T> pit) {
    ...
    if (at.isAnnotationPresent(MBean.class)) {
      ...
      if (!mBeanAnnotation.autoRegister()) {
        System.out.println(at.getJavaClass().getName() + "has the MBean annotation with autoRegister=false");
        return;
      }

      System.out.println("adding automatic JMX registration for: " + at.getJavaClass().getName());
