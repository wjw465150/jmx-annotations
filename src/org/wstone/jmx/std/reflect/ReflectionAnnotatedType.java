...
import org.wstone.jmx.std.reflect.BaseType.ClassFill;
import org.wstone.jmx.std.util.L10N;
  ...
  implements AnnotatedType<T>
{
  ...
  ReflectionAnnotatedType(BaseTypeFactory manager, 
                          BaseType type)
  {
    super(type,
  ...
  private void introspectSpecializesAnnotations(Class<?> cl)
  {
    ...
    if (cl.isAnnotationPresent(Named.class))
      throw new RuntimeException(L.l("'{0}' is an invalid @Specializes bean because it has a @Named annotation.",
                                    cl.getName()));
